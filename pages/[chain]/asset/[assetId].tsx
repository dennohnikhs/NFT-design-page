...
const IndexPage: NextPage<Props> = ({ assetId, ssr }) => {
  ...
  const { data: userTokens } = useUserTokens(
    is1155 ? connectedWallet?connectedWallet:undefined : undefined,
    {
      tokens: [`${contract}:${id}`],
    }
  )
  ...
  if (is1155) {
    ...
    countOwned =
      token?.token?.owner?.toLowerCase() === connectedWallet?.toLowerCase()
        ? 1
        : 0
  }

  const isOwner = countOwned > 0
  const owner = isOwner ? connectedWallet : token?.token?.owner
